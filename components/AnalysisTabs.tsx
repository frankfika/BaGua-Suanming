...
const AnalysisTabs: React.FC<AnalysisTabsProps> = ({ 
    ...
    const tabs = [
        { key: 'personality', label: '性情 (Self)', data: analysis.personality },
        { key: 'career', label: '事业 (Career)', data: analysis.career },
        { key: 'wealth', label: '财运 (Wealth)', data: analysis.wealth },
        { key: 'relationships', label: '情感 (Love)', data: analysis.relationships },
        { key: 'health', label: '健康 (Health)', data: analysis.health },
        { key: 'fortune', label: '大运 (Fortune)', data: analysis.globalFortune },
    ];
                    ...
                    <div className="animate-fade-in space-y-6 sm:space-y-8">
                                    ...
                                    {activeData.predictions.map((pred, i) => (
                                        <div key={i} className="bg-red-50/50 p-3 sm:p-4 rounded-lg border border-red-100 flex flex-col sm:flex-row gap-2 sm:gap-4 items-start">
                                            <div className="bg-white text-red-800 text-[10px] sm:text-xs font-bold px-2 py-1 rounded border border-red-100 whitespace-nowrap">
                                                {pred.match(/^\d{4}/) ? pred.substring(0, 4) : 'Future'}
                                            </div>
                        ...
                        {activeTab === 'personality' && (
                             <div className="mt-8 pt-6 border-t border-stone-200">
                                <h4 className="font-serif font-bold text-stone-800 mb-4 text-center">命理锦囊 (Cosmic Tips)</h4>
